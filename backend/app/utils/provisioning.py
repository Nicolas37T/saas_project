...
def create_tenant_schema(schema_name: str) -> bool:
    ...
    try:
        ...
        from app.db.tenant_models import Role, User, Setting, Patient, Treatment, MedicalHistory, Appointment, Payment, Odontogram, PatientShare, Medicine, TreatmentCatalog
        
        with engine.connect() as sqla_conn:
            sqla_conn.execute(text(f'SET search_path TO "{schema_name}", public'))
            
            tenant_models = [Role, User, Setting, Patient, Treatment, MedicalHistory, Appointment, Payment, Odontogram, PatientShare, Medicine, TreatmentCatalog]
